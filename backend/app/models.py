...
member_books = db.Table('member_books',
    db.Column('member_id', db.Integer(), db.ForeignKey('member.id')),
    db.Column('book_id', db.Integer(), db.ForeignKey('book.id'))
)


membership_books = db.Table('membership_books',
...
genres_books = db.Table('genres_books',
...
class Member(User):
    ...
    # Relationships
    reading_history = db.relationship('Book', secondary=member_books, backref='member', lazy='dynamic')
...
class Wishlist(db.Model, CRUDMixin):
    id = db.Column(db.Integer, primary_key=True)
    member_id = db.Column(db.Integer, db.ForeignKey('user.id'))

class Collection(db.Model, CRUDMixin):
    id = db.Column(db.Integer, primary_key=True)
...
class Book(db.Model, CRUDMixin):
    ...
    authors = db.relationship("Author", secondary=authors_books, backref="books", lazy='dynamic')
    genres = db.relationship('Genre', secondary=genres_books, backref='books', lazy='dynamic')
    collections = db.relationship('Collection', secondary=collection_books, backref='books', lazy='dynamic')
    wishlists = db.relationship("Wishlist", secondary=wishlist_books, backref="book", lazy="dynamic")
    free_access_in_memberships = db.relationship("Membership", secondary=membership_books, backref="book", lazy="dynamic")
...
class BookLoan(db.Model, CRUDMixin):
    id = db.Column(db.Integer, primary_key=True)
    book_id = db.Column(db.Integer, db.ForeignKey("book.id"), nullable=False)
    member_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)
    ...
    status = db.Column(db.String(20), default="issued")  # Statuses like issued, returned, overdue
